...
export default function Dancer({
	...
	const panResponder = useRef(
		PanResponder.create({
			// 주어진 터치이벤트에 반응할지 결정
			onStartShouldSetPanResponder: () => selectedPosTime !== undefined,
			...
			onPanResponderGrant: (event, gesture) => {
				...
				curPosAnimated.setOffset({ x: this.curVal.x, y: this.curVal.y })
			...
			onPanResponderRelease: (e, gesture) => {
				...
				curPosAnimated.setOffset({ x: 0, y: 0 });
				changeDancerPosition(dancer.did, this.curVal.x, this.curVal.y);
			}
		})
	).current;
